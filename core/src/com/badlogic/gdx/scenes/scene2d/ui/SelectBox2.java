...
public class SelectBox2<T> extends Widget implements Disableable {
	...
	public SelectBox2(Skin skin) {
		this(skin.get(SelectBoxStyle.class));
	}

	public SelectBox2(Skin skin, String styleName) {
		this(skin.get(styleName, SelectBoxStyle.class));
	}

	public SelectBox2(SelectBoxStyle style) {
		setStyle(style);
	...
	public void layout() {
		...
		prefWidth = Math.max(
				prefWidth,
				maxItemWidth
						+ (scrollStyle.background == null ? 0 : scrollStyle.background.getLeftWidth() + scrollStyle.background.getRightWidth())
						+ listStyle.selection.getLeftWidth()
						+ listStyle.selection.getRightWidth()
						+ Math.max(style.scrollStyle.vScroll != null ? style.scrollStyle.vScroll.getMinWidth() : 0, style.scrollStyle.vScrollKnob != null ? style.scrollStyle.vScrollKnob.getMinWidth()
								: 0));
	}
	...
	protected void onShow(Actor selectBoxList, boolean below) {
		...
		selectBoxList.addAction(fadeIn(0f, Interpolation.fade));
	}

	protected void onHide(Actor selectBoxList) {
		selectBoxList.getColor().a = 1;
		selectBoxList.addAction(sequence(Actions.alpha(1f), removeActor()));
	}

	/** @author Nathan Sweet */
	static class SelectBoxList<T> extends ScrollPane {
		private final SelectBox2<T> selectBox;
		int maxListCount;
		...
		public SelectBoxList(final SelectBox2<T> selectBox) {
	...
	/**
	 * The style for a select box, see {@link SelectBox2}.
	 * 
	 * @author mzechner
	 * @author Nathan Sweet
	 */
